# :world_map: Roadmap

I made this roadmap, just for development of things that are crucial for this game. When I complete all of these, I will add more features and content, but for now, this is the bare minimum for a playable game.

## Development

### Completed Tasks

* [x] Implement basic kart physics and controls.
* [x] Add sound effects and music.
* [x] Add .glb files for track (formerly .obj).
* [x] Add drifting, and a more robust physics system.
* [x] Add character models and animations.
* [x] Add tree models scattered around without lagging.
* [x] Add .ttf font and replace current font assets.
* [x] Add logo and put it into the game.
* [x] Implement a suspension system for better terrain handling.
* [x] Implement power-up system (boosts, projectiles, obstacles) by item capsules.

### In Progress & Upcoming

* [ ] Remodel the entire track, now with using .sskrt files.
* [ ] Redesing the UI HUD, now with redesigned speedometer, a lap counter, some changes, etc.
* [ ] Redesign the pause screen, now with moving stripes for the bg, and with some shadows.
* [ ] Add windowed and fullscreen mode.
* [ ] Add an exit button for those who want to exit the game on fullscreen.
* [ ] Add a "Results" screen after each race.
* [ ] Design a "Mini-map" system to track player.
* [ ] Add 4 different game modes: Grand Prix, Time Trial, Versus, and Battle Mode.
* [ ] Add more power-ups and items (missiles, shields, speed boosts).
* [ ] Add more tracks with unique themes and layouts.
* [ ] Design and implement multiple race tracks.
* [ ] Add more characters and karts with unique stats.
* [ ] Add local split-screen or online multiplayer support.
* [ ] Add debug system by Konami Code.
* [ ] Add ramps, jumps, and shortcuts to tracks.
* [ ] Implement a track editor and custom track support.
* [ ] Remaster the karts with using .sskrk files.
* [ ] Add a "Credits" screen with the original Scratch Team and Lucasliu9595.
* [ ] Add a "How to Play" screen with instructions.
* [ ] Implement AI opponents with waypoint pathfinding.
* [ ] Implement AI defensive/offensive logic (using power-ups strategically).
* [ ] Add environmental hazards (oil spills, falling rocks).
* [ ] Add dynamic lighting and custom shaders (smoke, sparks).
* [ ] Implement a weather system (rain, fog) that affects track conditions.
* [ ] Create a particle system for mud, grass, and drift marks.
* [ ] Remaster the physics for the kart's suspension, and add a more robust collision system.
* [ ] Add a "Photo Mode" to take in-game screenshots with custom camera angles.
* [ ] Implement a dynamic camera system that adjusts based on speed and track position.
* [ ] Add a "Gliding" mechanic for jumps and ramps.
* [ ] Implement a camera shake system for high speeds and collisions.
* [ ] Optimize performance and fix physics engine bugs.
* [ ] Separate the codebase into modules for better maintainability and scalability.
* [ ] Add crash handler and error logging system.
* [ ] Implement a "Cutscenes" system for intros and outros.
* [ ] Add more sound effects and music tracks, including character voices.
* [ ] Integrate a save system for high scores and unlocks.
* [ ] Add settings menu (audio, graphics) and credits.
* [ ] Add a ghost system to race against your best times.
* [ ] Add a character editor to customize your kart's appearance and stats.
* [ ] Remaster the font manager to add more fonts and support for different languages.
* [ ] Add a "Video Mode" to record gameplay footage, with using ffmpeg.
* [ ] Implement a "Replay System" to watch previous races.
* [ ] Add a main menu screen, game/player/map select.
* [ ] Implement a localization system for multi-language support.
* [ ] Enhance the AI with machine learning techniques for adaptive difficulty.
* [ ] Optimize the game for better performance on lower-end hardware.
* [ ] Implement a MMO mode with persistent online worlds and player interactions.
* [ ] Add an achievement and trophy system.
* [ ] Add a mods and addons system.
* [ ] Enhance the track editor with a user-friendly interface and more customization options.
* [ ] Final touches, and end roadmap.
